...
public class ResourcesService {
  ...
  public ResourcesService(InMemoryResourceStore store) { this.store = store; }

  public List<ResourceDTO> list(Optional<String> category, Optional<String> q) {
    if (category.isPresent() || q.isPresent()) return store.findByFilters(category, q);
    return store.findAll();
  }

  public Optional<ResourceDTO> get(String id) { return store.findById(id); }
}
