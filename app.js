...
/* Utilities */
function buildUrl() {
  // Approved public, unauthenticated endpoint
  return "https://jsonplaceholder.typicode.com/users";
}

function setStatus(msg, type = "info") {
  // type can be: info | success | error | empty | loading
...
async function fetchUsers() {
  try {
    ...
    // Render first 10 or all if fewer
...
function isValidEmail(value) {
  // Simple, reasonable check; browsers also validate type="email"
  const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
